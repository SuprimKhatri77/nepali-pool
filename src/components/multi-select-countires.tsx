...
export default function MultiSelectCountries({
    ...
    const filteredCountries = countries.filter(
        (country) => country.toLowerCase().includes(searchTerm.toLowerCase()) && !selectedCountries.includes(country),
    )
    ...
    const handleCountryRemove = (countryToRemove: string) => {
        onSelectionChange(selectedCountries.filter((country) => country !== countryToRemove))
    }
            ...
            {selectedCountries.length > 0 && (
                <div className="flex flex-wrap gap-2 mb-3 p-3 bg-muted/50 rounded-md">
                    {selectedCountries.map((country) => (
                        <Badge key={country} variant="secondary" className="flex items-center gap-1 px-2 py-1">
                            {country}
                            <Button
                                type="button"
                                variant="ghost"
                    ...
                    <div className="absolute z-50 w-full mt-1 bg-popover border rounded-md shadow-md max-h-60 overflow-auto">
                        {filteredCountries.length > 0 ? (
                            ...
                            <div className="px-3 py-2 text-sm text-muted-foreground">
                                {searchTerm ? "No countries found" : "Start typing to search countries"}
                            </div>
                        )}
                    </div>
                )}
