...
export default function ConnectStudent({
  ...
  const studyLevel: ConnectStudentProfileSelectType["studyLevel"] =
    (searchQuery.get(
      "studyLevel",
    ) as ConnectStudentProfileSelectType["studyLevel"]) ?? "";
  const intake =
    (searchQuery.get(
      "intake",
  ...
  const filterStudents = useMemo(() => {
    return students?.filter((std) => {
      if (
        city &&
        std.cityAppliedTo.trim().toLocaleLowerCase() !==
      ...
      if (
        studyLevel &&
        std.studyLevel.toLocaleLowerCase() !== studyLevel.toLocaleLowerCase()
      ) {
        return false;
      }
      if (
      ...
      if (
        intake &&
        std.intakeMonth.toLocaleLowerCase() !== intake.toLocaleLowerCase()
      ) {
        return false;
      }
      return true;
            ...
            <div className="flex md:flex-row flex-col space-y-4 justify-between px-4">
              <div className="flex relative">
                <Input
                  ...
                  placeholder="Search using school name"
                  className="min-w-[240px] sm:min-w-xs lg:min-w-2xl"
                />
