...
export default function SearchBelowHero({ mentors }: SearchBelowHeroProps) {
  ...
  return (
    <div ref={wrapperRef} className="w-full max-w-5xl  mt-8 mx-auto">
       {/* Search Bar */}
      <Command className="flex-1 sm:w-1/2 mx-auto my-4  border border-gray-200 rounded-xl shadow-sm hover:shadow-md transition-all duration-300 focus-within:border-emerald-300 bg-white">
          <CommandInput
  ...
  {results.map((m) => (
    ...
    >
      ...
      <div className="flex-1">
        <p className="text-sm font-medium text-gray-800">{capitalizeFirstLetter(m.user?.name ?? "")}</p>
        
        <p className="text-xs text-gray-400 line-clamp-1">{capitalizeFirstLetter(m.bio ?? "")}</p>
      </div>

      {/* Optional Badge or Tag */}
      <span className="text-[11px] text-emerald-600 font-semibold bg-emerald-50 px-2 py-1 rounded-lg">
        {capitalizeFirstLetter(m.city ?? "")}, {capitalizeFirstLetter(m.country ?? "")}
      </span>
    </CommandItem>
  ))}
</CommandGroup>
