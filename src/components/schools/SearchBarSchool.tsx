...
export default function SearchBarSchool({ schools }: SearchBelowHeroProps) {
  ...
  return (
    <motion.div initial={{scale:0, opacity:0}} animate={{scale:1, opacity:1}} transition={{duration: 0.8}} ref={wrapperRef} className="w-full max-w-5xl  mt-8 mx-auto sm:px-0 px-3">
       {/* Search Bar */}
  ...
  {results.map((s) => (
    ...
    >
      ...
      <div className="flex-1">
        <p className="text-sm font-medium text-gray-800">{capitalizeFirstLetter(s.name ?? "")}</p>
        
        <p className="text-xs text-gray-400 line-clamp-1">{capitalizeFirstLetter( s.websiteUrl ?? "")}</p>
      </div>
