...
export default function AnnouncementBanner({isVisible, setIsVisible, isAlreadyBooked}:{isVisible: boolean, setIsVisible: (visible: boolean) => void, isAlreadyBooked: unknown | null}) {
>>>>>>> origin/main
...
=======
      {isVisible && isAlreadyBooked == null ? (
...
=======
        ...
        isVisible && (
          <motion.div
  id="announ"
  key={"announcement-after-booking"}
  initial={{ y: -100, opacity: 0 }}
  animate={{ y: 0, opacity: 1 }}
  exit={{ y: -100, opacity: 0 }}
  transition={{ duration: 0.5, ease: "easeInOut" }}
  className="bg-gradient-to-r from-green-50 to-emerald-50 border-b border-emerald-200"
>
  <div className="max-w-[1350px] mx-auto px-4 sm:px-6 lg:px-8">
    <div className="py-3 sm:py-4">
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-3 flex-1 text-center sm:text-left">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-2">
            <div>
              <span className="text-sm sm:text-base font-semibold text-emerald-900">
                You’re all set for the Japan Session 🎉
              </span>
              <div className="flex items-center justify-center sm:justify-start gap-4 mt-1 text-xs sm:text-sm text-emerald-700">
                <div className="flex items-center gap-1">
                  <Calendar className="w-4 h-4" />
        ...
        <div className="flex items-center gap-3">
          <Link
            href="/sessions/live" // update it will google meet link
            className="px-5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium rounded-lg transition-all duration-200 shadow-sm hover:shadow-md active:scale-95"
          >
            Join Session
          </Link>
